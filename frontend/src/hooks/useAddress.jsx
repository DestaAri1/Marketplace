...
export default function useAddress() {
  ...
  const fetchProvinces = useCallback(async () => {
    const success = await handleAddressCall(() => addressAPI.getProvince());
    if (success.length > 0) {
  ...
  const fetchRegencies = useCallback(
    async (provinceId) => {
      ...
      const success = await handleAddressCall(() =>
        addressAPI.getRegencies(provinceId)
      );
      if (success.length > 0) {
  ...
  const fetchDistricts = useCallback(
    async (regencyId) => {
      ...
      const success = await handleAddressCall(() =>
        addressAPI.getDistricts(regencyId)
      );
      if (success.length > 0) {
  ...
  const fetchVillages = useCallback(
    async (districtId) => {
      ...
      const success = await handleAddressCall(() =>
        addressAPI.getVillages(districtId)
      );
      if (success.length > 0) {
        setVillages(success);
      } else {
