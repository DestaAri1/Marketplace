...
export default function UserList({user, onUpdate, onDelete}) {
    ...
    const getStatusColor = (status) => {
        switch(status) {
        case 0: return "bg-yellow-500"; // Unprocess
        case 1: return "bg-red-500";    // Rejected
        ...
        <table className="w-full border-collapse">
            ...
            {user && user.length > 0 ? (
                ...
                <tr key={index} className="border-b hover:bg-gray-100">
                    ...
                    <span
                        className={`px-3 py-1 rounded-full text-white text-xs font-semibold ${getStatusColor(user.status)}`}
                    >
